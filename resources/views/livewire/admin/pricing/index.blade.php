<div>
    ...
    <div class="flex justify-between items-center mb-6 mt-4">
        ...
        <a wire:navigate href="{{ route('admin.pricings.create') }}"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                ...
                class="w-5 h-5 me-2 icon icon-tabler icons-tabler-outline icon-tabler-coin">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg"> {{-- Inicio Tabla --}}
        <table
            ...
            <tbody>
                @forelse ($pricings as $pricing)
                    <tr
                        ...
                        <td class="px-6 py-3">
                            {{ $pricing->courtType->nombre }}
                        </td>
                        <td class="px-6 py-3">
                            {{ date('h:i A', strtotime($pricing->hora_inicio)) }}
                        </td>
                        <td class="px-6 py-3">
                            {{ date('h:i A', strtotime($pricing->hora_fin)) }}
                        </td>
                        <td class="px-6 py-3">
                            S/ {{ $pricing->precio_por_hora }}
                        </td>
                        <td class="px-6 py-3">
                            @if ($pricing->es_precio_nocturno)
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-sky-100 text-sky-800 dark:bg-sky-700 dark:text-sky-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round"
                                        class="w-5 h-5 me-1.5 icon icon-tabler icons-tabler-outline icon-tabler-moon">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                        <path
                            ...
                            @else
                                <span
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round"
                                        class="w-5 h-5 me-1.5 icon icon-tabler icons-tabler-outline icon-tabler-sun">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                        ...
                        <td class="px-6 py-3 text-right flex space-x-2">
                            <a wire:navigate href="{{ route('admin.pricings.edit', $pricing) }}"
                ...
                @empty
                    <tr>
                        <x-table-empty-state :colspan="7" :search="$search" title="{{ $titulo }}"
                            :createUrl="route('admin.pricings.create')" />
                    </tr>
                @endforelse
            </tbody>
        </table>

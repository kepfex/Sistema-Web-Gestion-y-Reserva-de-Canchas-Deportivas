<div>
    ...
    <div class="flex justify-between items-center mb-6 mt-4">
        ...
        <a wire:navigate href="{{ route('admin.schedules.create') }}"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                ...
                class="w-5 h-5 me-2 icon icon-tabler icons-tabler-outline icon-tabler-calendar-plus">
            ...
            <tbody>
                @forelse ($schedules as $schedule)
                    <tr
                        ...
                        <td class="px-6 py-3">
                            {{ $schedule->court->nombre }}
                        </td>
                        <td class="px-6 py-3 capitalize">
                            {{ $schedule->dia_de_la_semana }}
                        </td>
                        <td class="px-6 py-3">
                            {{ date('h:i A', strtotime($schedule->hora_apertura)) }}
                        </td>
                        <td class="px-6 py-3">
                            {{ date('h:i A', strtotime($schedule->hora_cierre)) }}
                        </td>
                        <td class="px-6 py-3 text-right flex space-x-2">
                            <a wire:navigate href="{{ route('admin.schedules.edit', $schedule) }}"
                ...
                @empty
                    <tr>
                        <x-table-empty-state :colspan="6" :search="$search" title="{{ $titulo }}"
                            :createUrl="route('admin.schedules.create')" />
                    </tr>
                @endforelse
            </tbody>
        </table>
